...
let playlist=[], currentPlaylistIndex=0, isPlayingYouTube=false;
...
// -------------------- YOUTUBE PLAYER --------------------
function onYouTubeIframeAPIReady(){ youtubePlayer = new YT.Player("youtube-player",{height:'315',width:'560',videoId:''}); youtubeReady=true; }
...
function updatePlaylistUI(){
  ...
  playlist.forEach((item,index)=>{
    ...
    removeBtn.textContent="X"; removeBtn.className="remove-btn"; removeBtn.onclick=(e)=>{ e.stopPropagation(); playlist.splice(index,1); updatePlaylistUI(); };
...
if(youtubePlayer) youtubePlayer.onStateChange = (e)=>{ if(e.data===YT.PlayerState.ENDED) playNextPlaylist(); }
...
function suggestYouTubeVideos(){
  ...
  if(playHistory.length>0){
    ...
    const keyword=last.name.split(" ")[0];
    suggestions.push(keyword+" song"); suggestions.push(keyword+" remix"); suggestions.push("top hits "+keyword);
  }else{ suggestions.push("top hits"); suggestions.push("popular music"); suggestions.push("trending music"); }
...
function initClient(){ gapi.client.init({apiKey:'', clientId:CLIENT_ID, scope:SCOPES}); }
loginBtn.addEventListener('click', async ()=>{ try{ const GoogleAuth=gapi.auth2.getAuthInstance(); await GoogleAuth.signIn(); fetchUserPlaylists(); }catch(err){console.error(err);} });
...
async function fetchPlaylistItems(playlistId){
  ...
  response.result.items.forEach(item=>{ const videoId=item.snippet.resourceId.videoId; const title=item.snippet.title; addYouTubeToPlaylist(videoId,title); });
}
...
// -------------------- VERSION CHECK --------------------
async function checkForUpdate(){ try{ const response=await fetch(VERSION_URL); const data=await response.json(); if(data.version!==CURRENT_VERSION){ if(confirm(`New version ${data.version} available. Download now?`)) window.location.href=data.apk; } }catch(err){console.log(err);} }
checkForUpdate();
